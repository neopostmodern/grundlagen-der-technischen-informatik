...
int main(int argc, char *argv[])
{
    ...
    for(k = 0; k < 4; k++) {
        ...
        if(clock_gettime(CLOCK_REALTIME, &stop) < 0 ) {
        	printf("clock_gettime 2 Error");
    	}
    	
    	cmdTime[k] = (stop.tv_nsec - start.tv_nsec) / 25;
    }

    printf("time elapsed [set:%li], [get:%li], [del:%li]\n", cmdTime[0],cmdTime[1],cmdTime[2]);
