...
struct sockaddr_in successor_their_addr, succ_successor_addr; // connector's address information
struct hostent *successor_he;
socklen_t successor_addrlen, succ_successor_addrlen;
...
int shouldFastForwardQuery(unsigned int key)
{
	int hash = hashKey(key) + 265 % 265;
	
	return hash >= 128 && hash < 256;
}
...
int isAddressQuery(char *commandName){
	return strcmp(commandName, "xad") == 0; 
}
...
void convertToInternalCommand(char *commandName) {	
	...
	} 	else if(isAddressQuery(commandName)){
		strcpy(commandName, "xad");
	}
}
