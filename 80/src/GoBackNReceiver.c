...
int main(int argc, char** argv) {
	...
	GoBackNMessageStruct* data = allocateGoBackNMessageStruct(DEFAULT_PAYLOAD_SIZE);
        ...
        if (data->hasErrors) {
          ...
          printf("Good: #%d\n", data->seqNo);
          long seqNoLong = (long) data->seqNo;
          if (lastReceivedSeqNo < seqNoLong) {
            printf("Possibly acceptable: #%li\n", seqNoLong);


            if (lastReceivedSeqNo + 1 == seqNoLong) {
              printf("Possibly acceptable: #%li\n", seqNoLong);

              goodBytes += bytesRead-sizeof(*data);
              writeBuffer(output, data);

              lastReceivedSeqNo = seqNoLong;

              if (data->size == sizeof(*data)) {
                printf(">>> TRANSMISSION COMPLETE.\n");
                printf("Good bytes: %lu", goodBytes);
                printf("Total bytes: %lu", totalBytes);
                fclose(output);
                complete = 1;
              }
            }
